...
template <typename T>
__global__ void col2im_gpu_kernel(const int n, const T* data_col,
                                  const int height, const int width,
...
#define INSTANTIATE_IM2COL_GPU(type)                                \
...
#define INSTANTIATE_COL2IM_GPU(type)                                \
  template void col2im<type>(                                       \
      const GPUDevice& d, const type* data_col, const int channels, \
      const int height, const int width, const int kernel_h,        \
      const int kernel_w, const int pad_h, const int pad_w,         \
      const int stride_h, const int stride_w, const int dilation_h, \
      const int dilation_w, type* data_im);

#define INSTANTIATE_IM2COL_COL2IM_GPU(type) \
  INSTANTIATE_IM2COL_GPU(type);             \
  INSTANTIATE_COL2IM_GPU(type);

INSTANTIATE_IM2COL_COL2IM_GPU(float);
INSTANTIATE_IM2COL_COL2IM_GPU(double);
INSTANTIATE_IM2COL_COL2IM_GPU(int);
INSTANTIATE_IM2COL_COL2IM_GPU(uint8);

#undef INSTANTIATE_IM2COL_GPU
#undef INSTANTIATE_COL2IM_GPU
#undef INSTANTIATE_IM2COL_COL2IM_GPU

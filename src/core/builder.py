...
def build_learning_rate(num_steps, global_step, lr_config, scope=None):
    ...
    with tf.name_scope(scope, 'learning_rate'):
        if lr_config['rate_type'] == 'pow_decay':

            cases = []
            cases.append((global_step > -1, lambda v=pow((1-global_step/num_steps),0.9): v))
            learning_rate_scale = tf.case(cases, lambda: tf.cast(0.0, tf.float64), exclusive=True)
...
def build_optimizer(optimizer_config, global_step):
    ...
    if opt_type == 'SGD':
        learning_rate = build_learning_rate(num_steps, global_step, lr_config)

...
def random_crop_and_pad_image_and_labels(image,
    ...
    class_label = class_label - ignore_label  # Needs to be subtracted and later added due to 0 padding.
    inst_label = tf.cast(inst_label, dtype=tf.float32)
    inst_label = inst_label - ignore_label  # Needs to be subtracted and later added due to 0 padding.
    ...
    last_inst_label_dim = tf.shape(inst_label)[-1]
...
def read_images_from_disk(input_queue, input_size, random_scale, random_mirror,
    ...
    img = tf.cast(
        tf.concat(axis=2, values=[img_b, img_g, img_r]), dtype=tf.float32)
    # Extract mean.
    img -= img_mean
    ...
    if input_size is not None:
        ...
        if random_scale:
            img, class_label, inst_label = image_scaling(
                img, class_label, inst_label)

        # Randomly mirror the images and labels.
        if random_mirror:
            img, class_label, inst_label = image_mirroring(
                img, class_label, inst_label)

        # Randomly crops the images and labels.
        img, class_label, inst_label = random_crop_and_pad_image_and_labels(
            img, class_label, inst_label, h, w, ignore_label)
...
class ImageReader(object):
    ...
    def __init__(self, data_dir, data_list, input_size, random_scale,
        ...
        Args:
          data_dir: path to the directory with images and masks.
          data_list: path to the file with lines of the form '/path/to/image
            /path/to/class/mask /path/to/class/inst_mask'.
          input_size: a tuple with (height, width) values, to which all the
            images will be resized.
          random_scale: whether to randomly scale the images prior to random
            crop.
          random_mirror: whether to randomly mirror the images prior to random
            crop.
          ignore_label: index of label to ignore during the training.
          img_mean: vector of mean colour values.
        '''
        ...
        self.class_labels = tf.convert_to_tensor(
            self.class_label_list, dtype=tf.string)
        self.inst_labels = tf.convert_to_tensor(
            self.inst_label_list, dtype=tf.string)
        self.queue = tf.train.slice_input_producer(
            [self.images, self.class_labels, self.inst_labels],
    ...
    def dequeue(self, num_elements):
        '''Pack images, class labels and instance labels into a batch.
        ...
        image_batch, class_label_batch, inst_label_batch = tf.train.batch(
            [self.image, self.class_label, self.inst_label], num_elements)
